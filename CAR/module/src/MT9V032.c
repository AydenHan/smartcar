...
uint8   image1[ROW][COL];      //图像数组
uint8   image2[ROW*COL] = {0};      //图像数组
uint8   Copy_Image[R35][L94] = {0};
...
int16 MT9V032_CFG[CONFIG_FINISH][2]=
{
    ...
    {UD_OFFSET,         100}, //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
...
void camera_init(void)
{
   ...
   // NVIC_SetPriority(MT9V032_INTERRUPT_NUNBERS,1);
...
void seekfree_sendimg_032(void)
{
  ...
  uart_putchar(UART1,0x01);//发送命令
  uart_putbuff(UART1, (uint8_t *)image, ROW*COL);  //发送图像
...
{
  unsigned int i, n = L94 * R35;
  for(i = 0; i < n; i++)
  b[i] = aa[i << 1];
}

void BinaryZation(unsigned char (*p)[L94])//二值化处理
{
    unsigned int i, j;
    for(i = 0; i < R35; i++)
      for(j = 0; j < L94; j++)
...
{
  Copy_Arr(image[0], Copy_Image[0]);//拷贝图像
  BinaryZation(Copy_Image);//二值化图像
  unsigned int i, j;
  for(i = 1; i < R35 - 1; i++)
  {
    for(j = 1;j < L94 - 1; j++)
