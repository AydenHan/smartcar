...
uint8 Stop_Flag = 0;
uint16 sensorFilter[5] = { 0 };	//阶梯滤波的电感值（useless）
uchar Flag_Round1 = 0;  //进入环岛的标志（在环岛里为ON）
uchar Flag_Round2 = 0;
uchar Flag_Round3 = 0;
uchar Flag_Round4 = 0;  //目前判断是向左拐还是向右拐
uchar Flag_Round5 = 0;
...
//水平电感偏差
	Direction_Err[0] = (float)(sensor[0]-sensor[1])/(sensor[0]+sensor[1]);//左-右水平电感的差比和作为偏差
...
//垂直电感偏差
        Direction_Err[1] = (float)(sensor[2]-sensor[3])/(sensor[2]+sensor[3]);//左-右水平电感的差比和作为偏差
...
{   
	...
	if (modestate == Mode3 && (sensor[1] > 66 && sensor[0] > 66))
	{
	...
	{
		if ((sensor[1] > 66 && sensor[0] > 66) && Flag_Round1 == 0)
		{
			Flag_Round3 = 1;
		}

		else if (sensor[0] < 50 && sensor[1] < 50 && Flag_Round3 == 1)
		{
			Flag_Round1 = Flag_Round3;
			Pwm_value = SERVO_MIDDLE + Pi + Di;
			Flag_Round2 = 1;
		}
		else if ((sensor[1] > 66 && sensor[0] > 66) && Flag_Round2 == 1)
		{
			Flag_Round4 = 1;
			Flag_Round3 = 0;
			Pwm_value = SERVO_MIDDLE + Pi + Di + Direction_Err[1] * 75;
		}
		else if (Flag_Round4 == 1 && sensor[0] < 50 && sensor[1] < 50)
		{
			Flag_Round5 = 1;
		}
		else if (sensor[1] > 50 && sensor[0] > 50 && modestate == Mode2)
		{
			Pwm_value = SERVO_MIDDLE + Pi + Di + Direction_Err[1] * 85;
		}
		else
		{
			Pwm_value = SERVO_MIDDLE + Pi + Di;
	...
	if (modestate == Mode3 && (sensor[1] < 50 && sensor[0] < 50) && electcamera)
	{
		car.incamera = 1;
		car.inelec = 0;
	}
    
	Pwm_value = (Pwm_value < 680 ? 680 : Pwm_value);
	Pwm_value = (Pwm_value > 830 ? 830 : Pwm_value);
}
...
void Send_Upper(void)
{
  ...
  sprintf((char*)a, "%d", speedlll);
  sprintf((char*)b, "%d", speedrrr);
